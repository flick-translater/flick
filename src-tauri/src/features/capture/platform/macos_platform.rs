...
pub fn hide_overlay_for_live_capture(app: &AppHandle, _state: &State<'_, AppState>) {
    eprintln!(
        "[long-capture/macos] hide_overlay_for_live_capture: order out overlay windows start"
    );
    let _ = frozen_overlay::order_out_overlay_windows(app);
    eprintln!(
        "[long-capture/macos] hide_overlay_for_live_capture: order out overlay windows complete"
    );
}
...
pub fn restore_overlay_after_live_capture(
    ...
    eprintln!(
        "[long-capture/macos] restore_overlay_after_live_capture: selection=({},{} {}x{}) start",
        selection.x, selection.y, selection.width, selection.height
    );
    let _ = frozen_overlay::restore_overlay_windows(app);
    let _ = update_highlight(app, Some(selection.clone()));
    eprintln!("[long-capture/macos] restore_overlay_after_live_capture: complete");
}

pub fn set_overlay_capture_sharing(app: &AppHandle, include_in_capture: bool) {
    eprintln!(
        "[long-capture/macos] set_overlay_capture_sharing: include_in_capture={include_in_capture}"
    );
...
pub fn set_overlay_mouse_passthrough(app: &AppHandle, passthrough: bool) {
    eprintln!("[long-capture/macos] set_overlay_mouse_passthrough: passthrough={passthrough}");
...
pub fn set_window_capture_sharing(window: &tauri::WebviewWindow, include_in_capture: bool) {
    let Ok(ns_window) = window.ns_window() else {
        eprintln!("[long-capture/macos] set_window_capture_sharing: missing ns_window");
        return;
    };
    ...
    window
        .run_on_main_thread(move || {
            ...
            eprintln!(
                "[long-capture/macos] set_window_capture_sharing: include_in_capture={include_in_capture}"
            );
        })
        .ok();
}
